...
function cloudguard_php_login_check() {
	...
	if ((in_array($GLOBALS['pagenow'], array('wp-login.php', 'wp-register.php'))) || ($xmlrpc && !$jetpack)) {
		...
		if (!in_array($attempt_country_code, $accepted_countries)) {
			...
			update_option('cloudguard_blocked_attempts', $blocked_attempts, 'no');

			// if redirect option set, redirect to url
			if (!empty($options['cloudguard_redirect']) && !$xmlrpc) {
...
function cloudguard_admin_assets($hook) {
	if (isset($_GET['page']) && $_GET['page'] == 'cloudguard') {
		...
		wp_enqueue_script('cloudguard-ammap-world', plugins_url('assets/ammap/maps/js/worldLow.js', __FILE__), array(), null, false);
	}
}
...
function cloudguard_code_to_country($code){
	...
	$country_list = array(
		...
		'ZM' => 'Zambia',
		'ZW' => 'Zimbabwe',
		'XX' => 'Unknown', // Cloudflare unknown location
	);
